...
import numpy as np
WORD_PER_SET = 5
with open("set_word/3000_oxford_words.txt", 'r') as f:
...
def take_screenshot():
    ...
    image = cv2.imread('images/screencap.png')
    # plt.imshow(image)
    # plt.show()
...
def get_create_study_set_position():
    pass
...
for x in range(number_of_set):
    ...
    with open(f'set_word/set_word_number_{x+1}','r') as f:
        list_word = f.readlines()
    for idx in range(x*WORD_PER_SET,x*WORD_PER_SET+WORD_PER_SET):
        take_screenshot()
        device.shell(f'input tap {x_add_phrase} {y_add_phrase}')
        device.shell(f'input tap {x_search_bar} {y_search_bar}')
        sleep(0.1)
